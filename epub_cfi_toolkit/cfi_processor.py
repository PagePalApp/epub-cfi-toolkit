...
class CFIProcessor:
    ...
    def __init__(self, epub_path: str) -> None:
        ...
        Raises:
            CFIError: If the EPUB file cannot be opened or is invalid
        """
    ...
    def extract_text_from_cfi_range(self, start_cfi: str, end_cfi: str) -> str:
        ...
        # Document structure validation is available via validate_cfi_bounds_strict method
        # Skipping here to maintain backward compatibility with existing extraction functionality
        ...
        if comparison > 0:
            raise CFIError(
                f"CFI range is in reverse order: start CFI '{start_cfi}' comes after end CFI '{end_cfi}'. "
                f"Please swap the start and end CFIs.")
        elif comparison == 0:
            return ""  # Same position returns empty string

        # Get the spine item and load the document (reusing for extraction)
    ...
    def _extract_text_between_positions(self, start_node, start_offset: int, start_type: str,
                                        end_node, end_offset: int, end_type: str) -> str:
        """
