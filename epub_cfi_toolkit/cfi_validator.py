...
class CFIValidator:
    ...
    def validate_against_document(self, cfi: str, epub_parser: "EPUBParser", document_tree: Optional["etree._Element"] = None) -> bool:
        """
    ...
    def validate_against_document_strict(self, cfi: str, epub_parser: "EPUBParser", document_tree: Optional["etree._Element"] = None) -> None:
        """
    ...
    def _validate_spine_references_strict(self, parsed_cfi, epub_parser: "EPUBParser") -> None:
        ...
        if not spine_item:
            ...
            raise CFIValidationError(
                f"CFI references spine item {itemref_step.index} but document only has {spine_count} spine items (valid range: 2-{spine_count * 2})")
        
        # Validate assertion if present
        if itemref_step.assertion and spine_item.id != itemref_step.assertion:
            raise CFIValidationError(
                f"CFI spine assertion mismatch: expected '{itemref_step.assertion}' but spine item {itemref_step.index} has id '{spine_item.id}'")
    ...
    def _validate_content_references_strict(self, parsed_cfi, document_tree: "etree._Element") -> None:
        ...
        for i, step in enumerate(parsed_cfi.content_steps):
            ...
            if child_index >= len(current_element):
                ...
                raise CFIValidationError(
                    f"CFI step /{step.index} references {ref_type} index {child_index} but element <{element_name}> at {element_location} only has {len(current_element)} children (valid range for step: 2-{len(current_element) * 2})")
        ...
        if parsed_cfi.content_steps:
            ...
            if last_step.index % 2 == 1:  # Text node reference
                ...
                if text_node_index < 0 or text_node_index >= len(text_nodes):
                    ...
                    element_location = ' -> '.join(element_path)
                    raise CFIValidationError(
                        f"CFI step /{last_step.index} references text node index {text_node_index} but element <{element_name}> at {element_location} only has {len(text_nodes)} text nodes")
